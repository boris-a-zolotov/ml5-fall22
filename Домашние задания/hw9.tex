...
противоречивым, а в противном случае~— непротиворечивым. Докажите следующее:\enumsep
	...
	\item Пусть $T$~— теория Хенкина, т.\:е. $T$ непротиворечива, любое предложение или его отрицание выводимо из $T$ и для любого выводимого из $T$ предложения вида $\exists x \psi(x)$ существует константный символ $c \in \sigma$ такой, что $T\vdash\psi(c)$. Докажите следующее:\enumsep
	    \item[(а)] $T\vdash\neg\varphi\iff T\not\vdash\varphi$. 
	    \item[(б)] $T\vdash(\varphi \lor \psi)\ \iff\ T\vdash\varphi$ или $T\vdash\psi$. 
	    \item[(в)] $T\vdash(\varphi \rightarrow \psi)\ \iff\ T\not\vdash\varphi$ или $T\vdash\psi$.
	    \item[(г)] $T\vdash\exists x \theta(x)\iff  T\vdash\theta(t)$ для
некоторого терма $t$ без переменных. 
	    \item[(д)] $T\vdash\forall x
